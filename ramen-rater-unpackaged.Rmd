...
```
Looking at these results we can see that The Ramen Rater's reviews have shifted slightly upward over time (F = 70.6, p < 0.001).
...
```
Looking at this figure, we might think that The Ramen Rater has rated box-style ramen the highest the most frequently. 
...
We know that The Ramen Rater does international reviews. I'm curious which country manufactures the highest rated ramens on average. 
...
kbl(sum_top.countries,
    caption = "Table 2. Top 10 countries with the highest average star rating.",
    align = c("l","r")) %>%
  kable_classic() %>%
  column_spec(1, bold = TRUE)%>%
    kable_styling(html_font = "lora_font")
...
I know I'll be scouring the internet for ways to get my hands on his 5-star picks, and who knows maybe I'll even start my own ramen-inspired blog. 
