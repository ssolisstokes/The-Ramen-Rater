...
```{r Star Distribution, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(data_ramen, aes(x = stars)) +
  ...
  theme_minimal(base_size = 14) +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold", margin = margin(t = 20, b = 20)), # hjust/margin: title placement
      axis.title.x = element_text(face = "bold", color = "#666", size = 13),
      axis.title.y = element_text(face = "bold", color = "#666", size = 13),
      axis.line = element_line(linewidth = 1),
      panel.grid.major.y = element_blank(),   # remove horizontal gridlines
      panel.grid.minor = element_blank()) +
  coord_flip()

ggsave("fig_ramen-star-distribution.png")
...
```{r Ratings Over Time - plot, echo=TRUE, message=TRUE, warning=TRUE}
model_review.stars <- plot(stars~review, data_ramen)
```

This is a bit hard to interpret let's try and simplify the visual.

```{r ggplot for Model, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(data_binned, aes(x = review.bin, y = mean.stars)) +
  ...
  theme(
    plot.title= element_text(hjust = 0.5, face = "bold", margin = margin(t = 20, b = 20)), # hjust & margin: title placement
  axis.title.x = element_text(face = "bold", color = "#666", size = 13),                   # formatting to look cohesive
  axis.title.y = element_text(face = "bold", color = "#666", size = 13),
  axis.line = element_line(linewidth = 1),
  panel.grid.major.x = element_blank())   

ggsave("fig_ramen-ratings-time.png")
```

Now we can look at what the statistics say to get to the bottom of **The Ramen Rater's** consistency.

```{r Model and Summary Stats, echo=TRUE, message=FALSE, warning=FALSE}
...
Let's dive a little deeper into **The Ramen Rater's** psyche, shall we? 

We can learn a bit about his preferences by looking at the different styles of ramen he's reviewed. 
There are 4 styles of ramen consumed and reviewed by **The Ramen Rater**: *cup, bowl, box and pack*. 

```{r Image Grid of Ramen Styles, message=FALSE, warning=FALSE, echo=FALSE, paged.print=FALSE}
...
```{r Styles and Preference, echo=FALSE, message=FALSE, warning=FALSE}
#### rating by style ####
...
ggplot(sum_style, aes(x = reorder(style, mean.stars), y = mean.stars)) +
  ...
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", margin = margin(t = 20, b = 20)),    # formatting for aesthetics
    axis.title.x = element_text(face = "bold", color = "#666", size = 13),
    axis.title.y = element_text(face = "bold", color = "#666", size = 13),
    axis.line = element_line(linewidth = 1),
    panel.grid.major.x = element_blank())   

ggsave("fig_ramen-style-rating.png")
...
```{r Styles and Preferece; NO BOX, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
ggplot(sum_style_no_box, aes(x = reorder(style, mean.stars), y = mean.stars)) +
  ...
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", margin = margin(t = 20, b = 20)),
    axis.title.x = element_text(face = "bold", color = "#666", size = 13),
    axis.title.y = element_text(face = "bold", color = "#666", size = 13),
    axis.line = element_line(linewidth = 1),
    panel.grid.major.x = element_blank())   

ggsave("fig_ramen-style-rating-nobox.png")
...
Okay, so we've learned that pack-style ramen is Queen of the Ramen Rater universe. But what about all the different *brands* **The Ramen Rater** has reviewed?  

Below is a table of the most consistently Ramen Rater loved pack-style brands for your next ramen hankerings.

```{r Pack by Brands Table, echo=FALSE, message=FALSE, warning=FALSE}
# summarise the data
sum_brands_pack <-
  data_ramen %>%
  filter(style == "Pack") %>%
...
Let's see:
```{r Top 10 Rated Countries, echo=FALSE, message=FALSE, warning=FALSE}
# summarise the data 
sum_top.countries <-
  data_ramen %>%
  group_by(country) %>%
...
As a newly indoctrinated *ramenphile* I want to know where the hidden gem ramens are. 

I've created a map that shows those countries which have a high star-rating average but also have less reviews overall to try and weed out those big countries (like the U.S and Japan). 

Within each of those high rated countries you'll also find info about the top rated ramen brand in that country to inspire your future ramen shopping list. 
```{r Hidden Gems Map, echo=FALSE, message=FALSE, warning=FALSE}
...
leaflet(gems_xy) %>%               
  addProviderTiles(providers$CartoDB.DarkMatter) %>%          # dark basemap
  addCircleMarkers(lng = ~long, lat = ~lat,                   # adding circles at "gems"
    radius = 7,
    stroke = TRUE, weight = 1,                                # formatting aesthetics
    color = "#993404",          
    fillColor = "#d95f02",    
    fillOpacity = 1.0,          
